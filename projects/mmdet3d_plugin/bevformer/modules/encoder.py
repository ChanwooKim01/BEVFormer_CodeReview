...
class BEVFormerEncoder(TransformerLayerSequence):
    ...
    def get_reference_points(H, W, Z=8, num_points_in_pillar=4, dim='3d', bs=1, device='cuda', dtype=torch.float):
        ...
        if dim == '3d':
            # 0.5 ~ z - 0.5 사이의 값에서 균등하게 4개의 점 샘플링
    ...
    def point_sampling(self, reference_points, pc_range,  img_metas):
        ...
        # reference_points를 실제 3D 좌표계로 변환 (0 ~ 1 -> pc_range)
        ...
        reference_points = reference_points.permute(1, 0, 2, 3) # [ㅇ, B, num_query, 4]
        ...
        # BEV 좌표계 상 reference points를 이미지 좌표계로 변환
        reference_points_cam = torch.matmul(lidar2img.to(torch.float32),
                                            reference_points.to(torch.float32)).squeeze(-1)
        eps = 1e-5

        # 각 이미지 좌표계 상 reference points가 이미지 영역 내에 있는지 확인 (z > eps 이상인 경우, 즉 z 값이 음수가 아닌 경우 이미지에 매칭된다 판단)
        bev_mask = (reference_points_cam[..., 2:3] > eps)
        # 동차 좌표계를 실제 이미지 좌표계로 scaling
        reference_points_cam = reference_points_cam[..., 0:2] / torch.maximum(
            reference_points_cam[..., 2:3], torch.ones_like(reference_points_cam[..., 2:3]) * eps)

        # 이미지의 H, W를 0 ~ 1 사이로 정규화
        ...
        if digit_version(TORCH_VERSION) >= digit_version('1.8'):
            bev_mask = torch.nan_to_num(bev_mask) # NaN → 0
        else:
    ...
    def forward(self,
        ...
        **kwargs contains some specific arguments of attentions.
        ...
        for layer in self.operation_order:
            ...
            if layer == 'self_attn':
                query = self.attentions[attn_index]( # TemporalSelfAttention
                    query,
                    prev_bev,
                    prev_bev,
        ...
        for layer in self.operation_order:
            ...
            if layer == 'self_attn':
                ...
                query = self.attentions[attn_index](
                    ...
                    key_padding_mask=query_key_padding_mask,
                    lidar_bev=lidar_bev,
                    reference_points=ref_2d, #none인 것 같은데...
                    spatial_shapes=torch.tensor(
                        [[bev_h, bev_w]], device=query.device),
                    level_start_index=torch.tensor([0], device=query.device),
                    **kwargs)
